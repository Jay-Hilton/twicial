...
class UserFriendshipsControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendship, user: users(:jacob), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:jacob), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
  ...
  context "#new" do
    ...
    context "when logged in" do
      ...
      should "display the friend's name" do
        get :new, friend_id: users(:robert)
        assert_match /#{users(:robert).full_name}/, response.body
      end
      ...
      should "assign a new user friendship to the correct friend" do
        get :new, friend_id: users(:robert)
        assert_equal users(:robert), assigns(:user_friendships).friend
      end

      should "assign a new user friendship to the currently logged in user" do
        get :new, friend_id: users(:robert)
        assert_equal users(:jacob), assigns(:user_friendships).user
      end
      ...
      should "ask if you really want to friend the user" do
        get :new, friend_id: users(:robert)
        assert_match /Do you really want to friend #{users(:robert).full_name}?/, response.body
      end
    end
  end
    ...
    context "when logged in" do
      ...
      context "with a valid friend id" do
        ...
        should "assign a user_friendship object" do
          assert assigns(:user_friendships)
          assert_equal users(:jacob), assigns(:user_friendship).user
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:jacob), friend: @friend)
        create(:pending_user_friendship, friend: users(:jacob), user: @friend)
        sign_in users(:jacob)
  ...
  context "#edit" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:jacob))
        sign_in users(:jacob)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:jacob))
        create(:accepted_user_friendship, friend: users(:jacob), user: @friend)
        sign_in users(:jacob)
      end
